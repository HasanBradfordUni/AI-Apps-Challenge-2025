...
def download_session(session_id):
    ...
    try:
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        summary_filename = f"{session_id}_session_summary.txt"
        summary_path = os.path.join(code_processor.export_folder, summary_filename)
        
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("AI PROGRAMMING ASSISTANT - SESSION SUMMARY\n")
            f.write("="*80 + "\n\n")
            
            f.write(f"Session ID: {session_id}\n")
            f.write(f"Date: {session_data.get('timestamp', 'Unknown')}\n")
            f.write(f"Programming Language: {session_data.get('language', 'Unknown').upper()}\n")
            f.write(f"Assistance Type: {session_data.get('type', 'Unknown').replace('_', ' ').title()}\n\n")
            ...
            # Original code
            f.write("USER INPUT CODE:\n")
            f.write("-" * 40 + "\n")
            f.write(session_data.get('code_content', 'No code available') + "\n\n")
            
            # AI response
            f.write("AI ASSISTANT RESPONSE:\n")
            f.write("-" * 40 + "\n")
            f.write(session_data.get('suggestion', 'No suggestion available') + "\n\n")
            
            f.write("="*80 + "\n")
            f.write("Generated by Has AI - AI Programming Assistant\n")
            f.write("="*80 + "\n")
        
        print(f"Download file path: {summary_path}")
...
def download_session_format(session_id, export_format):
    ...
    try:
        ...
        if export_format not in ['txt', 'markdown', 'md']:
            return jsonify({'error': 'Invalid format. Only txt and markdown are supported'}), 400
        
        session_data = coding_sessions[session_id]
        
        if export_format == 'txt':
            return download_session(session_id)
        else:
            # Markdown format
            summary_filename = f"{session_id}_session_summary.md"
            summary_path = os.path.join(code_processor.export_folder, summary_filename)
...
def get_sessions():
    ...
    try:
        # Convert sessions dict to list format
        sessions_list = []
        for session_id, session_data in coding_sessions.items():
            sessions_list.append({
                'id': session_id,
                'code_content': session_data.get('code_content', ''),
                'language': session_data.get('language', 'unknown'),
                'suggestion': session_data.get('suggestion', ''),
                'timestamp': session_data.get('timestamp', ''),
                'type': session_data.get('type', 'unknown'),
                'assistance_settings': session_data.get('assistance_settings', {}),
                'code_length': len(session_data.get('code_content', ''))
            })
