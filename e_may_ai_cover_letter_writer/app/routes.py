...
def profile():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash('Welcome back! Loading your profile.', 'success')
            session['user_id'] = user[0]
            session['user_name'] = user[1]
            session['user_email'] = user[2]
            
            # Load existing CV data into session for display
            load_user_cv_data(user[0])
...
def load_user_cv_data(user_id):
    """Load user's existing CV data for display"""
    ...
    # Get education
    cursor.execute("SELECT institution, degree, field, start_date, end_date FROM user_education WHERE user_id = ?", (user_id,))
    education = cursor.fetchall()
    
    # Get experience
    cursor.execute("SELECT company, position, description, start_date, end_date FROM user_experience WHERE user_id = ?", (user_id,))
    experience = cursor.fetchall()
    
    # Store in session for easy access
    session['existing_skills'] = [{'skill_name': s[0], 'proficiency': s[1]} for s in skills]
    session['existing_education'] = [{'institution': e[0], 'degree': e[1], 'field': e[2], 'start_date': e[3], 'end_date': e[4]} for e in education]
    session['existing_experience'] = [{'company': ex[0], 'position': ex[1], 'exp_description': ex[2], 'start_date': ex[3], 'end_date': ex[4]} for ex in experience]
...
def cv_details():
    ...
    if request.method == 'POST':
        # Clear existing data for this user before adding new data
        user_id = session['user_id']
        cursor = connection.cursor()
        cursor.execute("DELETE FROM user_skills WHERE user_id = ?", (user_id,))
        cursor.execute("DELETE FROM user_education WHERE user_id = ?", (user_id,))
        cursor.execute("DELETE FROM user_experience WHERE user_id = ?", (user_id,))
        connection.commit()
...
def generate_letter():
    ...
    # Get CV text from both session and database
    cv_text_from_session = get_session_cv_text()
    
    # Get structured CV data from database
    user_id = session['user_id']
    cursor = connection.cursor()
    ...
    # Format CV data from database
    cv_text_from_db = format_cv_text(skills_data, education_data, experience_data)
    
    # Combine both CV sources
    combined_cv_text = cv_text_from_session + "\n\n" + cv_text_from_db if cv_text_from_session else cv_text_from_db
    
    # Generate cover letter with the combined CV data
...
def export_letter(letter_id, format):
    ...
    # letter_data is a tuple, need to access by index
    cover_letter = letter_data[5]  # generated_letter column
    job_title = letter_data[2]     # job_title column
    company_name = letter_data[3]  # company_name column
    
    filename = f"Cover_Letter_{job_title.replace(' ', '_')}"
    ...
    if format == 'docx':
        ...
        try:
            ...
            # Title
            title = Paragraph(f"Cover Letter: {job_title}", styles['Title'])
            story.append(title)
            story.append(Spacer(1, 0.2*inch))
            
            # Company
            company = Paragraph(company_name, styles['Heading2'])
            ...
            p.drawString(50, height - 50, f"Cover Letter: {job_title}")
            
            # Company
            p.setFont("Helvetica-Bold", 12)
            p.drawString(50, height - 80, company_name)
        ...
        txt_content = f"Cover Letter: {job_title}\n"
        txt_content += f"{company_name}\n\n"
